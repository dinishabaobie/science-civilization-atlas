...
纸面不再固定重复同一套栏目，而是按节点生成五种阅读镜头：知识转向、能力转向、组织转向、关键耦合与系统风险。每种镜头重新安排转折、行动者、落地条件与边界的顺序和标题，并显示两种燃料、当前闸门、历史规律与本节点追问。纸面是唯一允许长段宋体阅读和显著环境阴影的区域。

### Long-form Manuscript

时间轴之后的母稿按十章组织：三个概念、统一框架、四条主线、八次耦合、七条规律、跨域者、技术十六问、六项批判、文明观、四书与边界。桌面目录使用五列两行，不设置悬浮侧栏，避免长文阅读再次被固定导航挤压；章节内部使用边线、序列和原生 `details` 做渐进展开。
